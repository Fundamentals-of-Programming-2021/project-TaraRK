...
{ 
    ...
    // testr.h = 200;
    // testr.w = 200;
    SDL_bool enternewgame = SDL_FALSE;
    SDL_bool enterpreviousgame = SDL_FALSE;
    SDL_bool enterscoreboard = SDL_FALSE;
    ...
    SDL_Renderer *sdlRenderer = SDL_CreateRenderer(sdlWindow, -1, 
                                                   SDL_RENDERER_PRESENTVSYNC | 
                                                   SDL_RENDERER_ACCELERATED);
    if (validate(&begingame, &previousgame, &Scoreboard, &Menumouse) == 1)
                    {
